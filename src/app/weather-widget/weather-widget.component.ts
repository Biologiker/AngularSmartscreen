...
export class WeatherWidgetComponent implements OnInit {
  ngOnInit() {
    ...
    refreshSeconds('second', currDate.getSeconds().toString(), currDate.getMilliseconds() >= 500 /*Dieses If schaltet doMove an oder aus damit der Rythmus des Switchens gleich bleibt*/, true);

    refreshSeconds('minute', currDate.getMinutes().toString(), currDate.getMilliseconds() >= 500, document.getElementById("secondLeft")!.classList.contains('move'));

    refreshSeconds('hour', currDate.getHours().toString(), currDate.getMilliseconds() >= 500, document.getElementById("secondLeft")!.classList.contains('move'));
...
function refreshSeconds(element: string, timeElement: string, doMove: boolean, parentHasMove: boolean) { //NOSONAR
...
function refreshWrapper(currDate: Date) {
  refreshSeconds('second', currDate.getSeconds().toString(), true, true);

  if (currDate.getSeconds() == 59 || currDate.getSeconds() == 0) {
    refreshSeconds('minute', currDate.getMinutes().toString(), true, document.getElementById("secondLeft")!.classList.contains('move'));

    if (currDate.getMinutes() == 59 || currDate.getMinutes() == 0) {
      refreshSeconds('hour', currDate.getHours().toString(), true, document.getElementById("secondLeft")!.classList.contains('move'));
    }
  }
}